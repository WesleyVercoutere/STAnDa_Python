...
class Chart2:
    ...
    def _setAx(self, ax, index):
        ...
        self._lines.append(ax.plot(data[index].dfReferenceAvg["time"], data[index].dfReferenceAvg["avg_tpm"], label="Average tpm")[0])
        self._lines.append(ax.plot(data[index].dfReferenceAvg["time"], data[index].dfReferenceAvg["upper_boundary"], label="Upper limit")[0])
        self._lines.append(ax.plot(data[index].dfReferenceAvg["time"], data[index].dfReferenceAvg["lower_boundary"], label="Lower limit")[0])
        self._lines.append(ax.plot(data[index].dfMonitorAvg["time"], data[index].dfMonitorAvg["avg_tpm"], label=f"Avg {self._settings.nbrOfValuesForMonAvg} samples")[0])
        self._lines.append(ax.plot(data[index].dfMonitorMed1["time"], data[index].dfMonitorMed1["median"], label=f"Median {self._settings.nbrOfValuesForMed1} samples")[0])
