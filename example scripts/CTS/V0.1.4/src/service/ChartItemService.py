...
class ChartItemService:
    ...
    def processData(self):
        ...
        [self._addReferenceMed(chartItem) for chartItem in self._chartItemRepo.getAll()]
    ...
    def _addReferenceMed(self, chartItem: ChartItem):
        df = chartItem.dfSignalData.head(self._settings.nbrOfValuesForRefAvg)
        med = df["tpm"].median()

        chartItem.dfReferenceMed = chartItem.dfSignalData.head(1)
        chartItem.dfReferenceMed = chartItem.dfReferenceMed.append(chartItem.dfSignalData.tail(1))
        chartItem.dfReferenceMed = chartItem.dfReferenceMed.filter(["time"]).reset_index(drop=True)

        chartItem.dfReferenceMed["med_tpm"] = med
        chartItem.dfReferenceMed["lower_boundary"] = chartItem.dfReferenceMed["med_tpm"] * (1 - (self._settings.boundary / 100))
        chartItem.dfReferenceMed["upper_boundary"] = chartItem.dfReferenceMed["med_tpm"] * (1 + (self._settings.boundary / 100))
