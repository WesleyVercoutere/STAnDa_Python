...
class DataService:
    ...
    def processData(self):
        ...
        data = data.dropna(subset=["tpm"], how='any', axis=0)
        data.reset_index(drop=True)

        filteredData = data.filter(["time", "tpm", "V"])
    ...
    def _addAvgTpm(self, data):
        data["tpm"] = data.apply(lambda x: self._calculateAverage(x), axis=1).reset_index(drop=True)

    def _calculateAverage(self, row):
        values = []
        prefixFreq = "f"
        prefixFFT = "FT"
