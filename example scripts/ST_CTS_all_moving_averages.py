...
class Chart:
    ...
    def splitData(self):
        ...
        for id in self.idSet:
            ...
            self.dfChartS[id] = groupbyTwist.get_group("TwistS_")
            self.dfChartZ[id] = groupbyTwist.get_group("TwistZ_")
    ...
    def showSubPlot(self):
        ...
        for item in self.dfChartS:
            ax1.plot(self.dfChartS[item]['time'], self.dfChartS[item]['MovingAvg'], label = item)
        ...
        for item in self.dfChartZ:
            ax2.plot(self.dfChartZ[item]['time'], self.dfChartZ[item]['MovingAvg'], label = item)
